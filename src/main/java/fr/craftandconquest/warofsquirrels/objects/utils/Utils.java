...
public class Utils {
    ...
    public static boolean       checkCityName(String name) {
        ...
        String                  sql = "SELECT `" + GlobalCity.displayName + "` FROM `" + GlobalCity.tableName + "` WHERE `" + GlobalCity.displayName + "` = ?;";
    ...
    public static Location<World>   getNearestSpawn(DBPlayer player) {
        ...
        for (Chunk c : chunks) {
            if (c.getWorld().getUniqueId().equals(player.getUser().getPlayer().get().getWorld().getUniqueId())) {
                Vector3d vec = new Vector3d(
                        c.getRespawnX(),
                        c.getRespawnY(),
                        c.getRespawnZ());
                if (save != null) Core.getLogger().warn(pLocation.getPosition().distance(save.getPosition()) + " > " + pLocation.getPosition().distance(vec));
    ...
    */
    ...
    public static boolean checkShopFormat(SignData datas) {
        ...
        return patternHeader.matcher(tag.toPlain()).matches() &&
    ...
    public static boolean checkLoanFormat(SignData datas) {
        Text header = datas.lines().get(0);
        Text cubo = datas.lines().get(1);
        Text buy = datas.lines().get(2);
        Text rent = datas.lines().get(3);
